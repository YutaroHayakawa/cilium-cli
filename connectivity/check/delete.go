...
func (ct *ConnectivityTest) generateManifestsNodeAffinity(ctx context.Context, helmState *helm.State) error {
	...
	printHelmTemplate := false

	vals, err := helm.MergeVals(
		// ConnectivityTest does not satisfy utils.Logger interface because its Log method
		// is different. Plumb a rapper
		logger{ct},
		printHelmTemplate,
		values.Options{},
		helmMapOpts,
